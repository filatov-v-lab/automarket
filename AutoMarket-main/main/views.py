...
from store.settings import BOT_TOKEN, CHAT_ID


# Create your views here.
def store(request):
    products = models.Product.objects.all()
    base_properties = models.ProductProperty.objects.values('base_property_id', 'base_property__name_ru', 'value').annotate(p_count=Count('value')).order_by('base_property_id')
    ordering = ''
...
def product(request, pk):
    ...
    recommendations = models.Product.objects.filter(category_id=product_obj.category.pk, price__lte=product_obj.price + product_obj.price / 2).exclude(pk=product_obj.pk).order_by('id')[:15]
...
def order_create(request):
    ...
    if order and BOT_TOKEN and CHAT_ID:
        products = "\n".join([f'{n} {str(q)} шт.' for n, q in [p for p in order.orderproduct_set.values_list('product__name', 'quantity')]])
        message = f'<b>Новый заказ</b>\n<b>От</b>: #{request.user.pk} {request.user.username}\n<b>Телефон</b>: {order.phone}\n<b>Адрес</b>: {order.address.get_address_str()}\n<b>Сумма</b>: {round(order.get_total(), 2)}\n<b>Покупки</b>:\n{products}\n\n<b>Комментарий</b>: {order.comment}'
...
def send_feedback(request):
    ...
    if request.POST:
        ...
        if feedback and BOT_TOKEN and CHAT_ID:
            message = f'<b>Новое обращение</b>\n<b>От</b>: #{feedback.name}\n<b>Телефон</b>: {feedback.phone}\n<b>Почта</b>: {feedback.email}\n<b>Текст обращения</b>: {feedback.comment}'
            requests.post(
                url=f'https://api.telegram.org/bot{BOT_TOKEN}/sendMessage',
                data={'chat_id': {CHAT_ID}, 'text': message, 'parse_mode': 'html'}
            )
...
def login_store(request):
    ...
    if request.POST and 'username' in request.POST and 'password' in request.POST:
        ...
        if user and user.is_authenticated:
            login(request, user)
            return redirect('/')
        return render(request, 'main/login.html', context={'error': 'Неверное имя пользователя и (или) пароль.'})
